...
const MyClub: React.FC<MyClubProps> = ({ user }) => {
  ...
  const fetchClubData = async () => {
    ...
    try {
        ...
        console.error("Error fetching my club from database:", e);
        setClubData(null);
    } finally {
        setLoading(false);
    }
  ...
  const handleAddMember = async (e: React.FormEvent) => {
      e.preventDefault();
      // Mocking the UI update for member addition as real user creation requires Admin Auth API
      if(newMemberName) {
          setMembers([...members, { 
              id: `temp-${Date.now()}`, 
              name: newMemberName, 
  ...
  if (!clubData) {
      return (
          <div className="pt-32 px-4 text-center">
              ...
              <p className="text-slate-500">You are not assigned to a club in the database.</p>
          </div>
      );
  }
    ...
    >
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-10 gap-4">
        <div>
           ...
           <p className="text-slate-500 mt-2">Oversee club activities and update details in real-time.</p>
        </div>
        <button 
              ...
              <div className="bg-white rounded-[2rem] p-6 shadow-sm border border-slate-100 h-full flex flex-col">
                  ...
                  <div className="space-y-4 flex-grow overflow-y-auto max-h-[500px] pr-2">
                      {members.length > 0 ? members.map((member) => (
                          ...
                          <div className="text-center py-8 text-slate-400 text-sm">No members found in database</div>
                      )}
                  </div>
              </div>
          </div>
      ...
      {showAddMemberModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/30 backdrop-blur-md">
            <div className="bg-white rounded-2xl shadow-2xl p-6 w-full max-w-sm animate-in zoom-in-95 duration-200 border border-white/50 relative">
                ...
                <h3 className="text-xl font-bold text-slate-900 mb-4">Add Member</h3>
